...
const Booking = () => {
  ...
  const handleEditSave = () => {
    axios
      .put(`http://localhost:5000/api/bookings/${editbooking._id}`, editbooking)
      .then((res) => {
  ...
  const navItems = [
    ...
    ...(userData?.role === "tour_guide"
      ? [
          {
            label: "Add Tourist booking",
            path: "/add-tourist-booking",
            icon: <User className="text-[16px]" />,
          },
          {
            label: "Tourist booking List",
            path: "/tourist-booking-list",
            icon: <User className="text-[16px]" />,
          },
          {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
            {bookings
              .filter((booking) => {
                ...
                // For Admin: show all with search
                if (userRole === "admin") return matchesSearch;

                // For Tour Guide: only their bookings with search
                return booking.guideId?._id === loggedInUserId && matchesSearch;
              })
              .slice(indexOfFirstHotel, indexOfLastHotel)
              .map((booking) => (
                <div
                  key={booking._id}
                  className="bg-white shadow-lg rounded-xl p-6 border border-blue-100 hover:shadow-2xl transition"
                >
      ...
      {editbooking && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-[90%] max-w-lg">
            ...
            />
            <label htmlFor="">Toutr Time:</label>
            <input
              type="text"
              className="w-full p-2 mb-3 rounded border border-blue-600"
